...
Next backend action should run one precise pin-track alignment probe with explicit layers such as {M1 M2}. If that also fails, move to a structural/cell-mapping alternative that reduces the problematic A2 OR/NOR pin-access situations without broad-banning NOR2X4_HVT. If a backend ECO trial helps, move the fix back into synthesis and re-run FE equivalence/DFT/PT before treating it as implementation evidence.
```
