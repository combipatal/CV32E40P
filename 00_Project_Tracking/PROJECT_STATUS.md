...
Full remaining-marker context after the A1/A2 pin-swap ECO was extracted. 95 of 103 remaining DRCs are still near already-swapped cells, and 99 markers are near A1 pins. The pin swap mostly moved the OR/NOR lower-metal access problem from A2 to A1. Pin-swap-only ECO is no longer the main closure strategy.
Next backend action should test a structural/cell-mapping alternative that reduces the problematic OR/NOR pin-access situations without broad-banning NOR2X4_HVT. If a backend ECO trial helps, move the fix back into synthesis and re-run FE equivalence/DFT/PT before treating it as implementation evidence.
```
