...
Next backend action should move to a structural/cell-mapping alternative or NDM/tech/via setup check that reduces the problematic A2 OR/NOR pin-access situations without broad-banning NOR2X4_HVT. If a backend ECO trial helps, move the fix back into synthesis and re-run FE equivalence/DFT/PT before treating it as implementation evidence.
```
